    ...
    <ul class=" lien2 nav navbar-nav navbar-right">
      ...
      <section>
         <div class="container-fluid admin">
           <div class="row">
             <h1 style="text-align:center" class="titr-market"><strong>LISTE DES INGREDIENTS</strong> <a href="insert.php" class="btn btn-warning btn-lg ajou"><span class="glyphicon glyphicon-plus"></span> AJOUTER</a></h1>
	 ...
	 // $bd=new PDO('mysql:host=sql.nsipaie.com;dbname=c3074_nsidb','c3074_cmoussa','NsiData2018');
	  ...
	  catch(exception $e)
	  {
		  die('Erreur:'.$e->getMessage());
			...
			// faire ine requete
		  //$req=$bd->query('SELECT `Id`, `nom`, `titr`, `im`, `prix`, `categorie` FROM `ingredient`') or die("erreur");
		  // afficher
		 //$result = $req->fetch();
		 // var_dump($req);
		  foreach($req=$bd->query('SELECT `Id`, `nom`, `titr`, `im`, `prix`, `categorie`,`nom_cat` FROM `ingredient` LEFT JOIN `categorie` ON `categorie`= `id_cat`  ORDER BY `Id` DESC') as $result)
		  {
			 
		       echo '<tr>';
               echo '<td>'.$result['nom'].'</td>'; 
               echo '<td>'.$result['titr'].'</td>'; 
               echo '<td>'.$result['prix'].'</td>';
			   echo '<td>'.$result['im'].'</td>'; 
               echo '<td>'.$result['nom_cat'].'</td>'; 
               echo '<td>
                      <a class="btn btn-default" href="voir.php?id='.$result['Id'].'"><span class="glyphicon glyphicon-eye-open"></span> Voir</a>&nbsp;
                      <a class="btn btn-primary" href="modif.php?id='.$result['Id'].'"><span class="glyphicon glyphicon-edit"></span> Modifier</a>&nbsp;
                      <a class="btn btn-danger" href="delete.php?id='.$result['Id'].'"><span class="glyphicon glyphicon-trash"></span> Supprimer</a>
