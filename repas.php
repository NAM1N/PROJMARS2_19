...
if (!empty($_POST))
{
    ...
    // insertion faire ine reque
		   $val=array($_POST["lib"],$tab["nbre"],$_SESSION["id_cli"],$_POST["pri"]);
  ...
  <div class="container-fluid">
    ...
    <ul class=" lien2 nav navbar-nav navbar-right">
      ...
      <section class="market" >
       <div class="container-fluid">
         <div class="row">
           <div class="col-md-12">
           <h1 class="titr-market"><span class=" couv glyphicon glyphicon-cutlery"></span> BONNE APPETI <span class=" couv glyphicon glyphicon-cutlery"></span></h1>
	...
	//   $bd=new PDO('mysql:host=sql.nsipaie.com;dbname=c3074_nsidb','C3074dbuRW','QUp_Ari8d');
	 // $bd=new PDO('mysql:host=sql.nsipaie.com;dbname=c3074_nsidb','c3074_cmoussa','NsiData2018');
	  ...
	  $req=$bd->query('SELECT * FROM `repastype`');
	  $cat=$req->fetchAll();
     ...
     echo '<nav class="menu-market">
		...
		foreach($cat as $result)
	   {
		   if($result["Id_repa"]=='1')
		   {
			   echo '<li role="presentation" class="active"><a href="#'.$result["Id_repa"].'" data-toggle="tab"><span></span>'. $result["cat_repa"].'</a></li>';
		   }else { 
		   echo '<li role="presentation"><a href="#'.$result["Id_repa"].'" data-toggle="tab"><span></span>'. $result["cat_repa"].'</a></li>';
		   }
	   }
		 echo '    
             </ul>
		    ...
		    foreach($cat as $result)
	   {
		  if($result["Id_repa"]=='1')
		   {
			   echo '<div class="tab-pane active " id="'.$result["Id_repa"].'" >';
		   }else echo '<div class="tab-pane  " id="'.$result["Id_repa"].'" >';
		   //echo '<div class="prix">'.$result["id_cat"].'</div>';
		   echo '<div class="row">';
	
			$req=$bd->prepare('SELECT `Id`, `titr`, `descri`, `prix`, `im`, `respastype`, `dat_inscr_cli` FROM `repas` WHERE `respastype`=?');
			$req->execute(array($result["Id_repa"]));
			//$cat1=$req->fetchAll();
			//var_dump($cat1);
			...
			while( $list=$req->fetch())
			   {
				 ...
				 echo '<div class="thumbnail">
                          ...
                          <img src="images/REPAS/'.$list["im"].'" style="width:100%" />
                          <div class="prix">'.$list["prix"].'FRS CFA</div>
                          <div class="caption">
                          <h4>'.$list["titr"].'</h4>
                          <p>'.$list["descri"].'</p>
						  <form method="post" action="command.php" enctype="">
                          <div class="form-group">
                              <input type="number" step="1" placeholder="lE NOMBRE DE PLATS"  class="form-control" required="required" name="nbre" value="1"/>
							  <input type="hidden" class="form-control"  name="lib" value="'.$list["titr"].'"/>
						...
						?>	
							  ...
							  <p style="display:<?php  if( $tab["valid"]) echo 'block'; else echo 'none';  ?>; font-weight:bold; " class=" glyphicon glyphicon-ok alert alert-success">Votre commande a &eacute;t&eacute; prise en compte</p>      
                  <?php 
                     echo '     </form>
						  </div>
