    ...
    <ul class=" lien2 nav navbar-nav navbar-right">
      ...
      <section>
         <div class="container-fluid admin">
           <div class="row">
             <h1 style="text-align:center" class="titr-market"><strong>MON AVIS SUR LA RECETTE </strong> </h1>
              <div class="col-md-8">
               <?php
			       try
		      {
			  ...
			  //les recttes
			  $recet=$bd->prepare('SELECT `Id_recet`, `titr_recet`, `detail`, `im`, `client`,`nom_cli`, `pnom_cli`, `dat_cli` FROM `recette` LEFT JOIN `client` ON `client`=`Id_cli` WHERE `Id_recet`=?');
			  $recet->execute(array($id));
			  $result=$recet->fetch();
                ...
                <?php 
				// affichage des commentatire
				$req1=$bd->prepare('SELECT `Id_comt`, `commentaire`, `recette`, `client`, `dat_comt`,`Id_cli`, `nom_cli`, `pnom_cli` FROM `coment` LEFT JOIN `client` ON `client`=`Id_cli` WHERE `recette`=? ORDER BY `Id_comt` DESC LIMIT 0,5 ' );
				 
			$req1->execute(array($_SESSION["recet"]));
			while($done=$req1->fetch())
			{
			
			  echo'<p><span style="font-weight:bold"></h3><img src="images/avatar2.png"/>'.$done["nom_cli"].''.$done["pnom_cli"].'</span> a r&eacute;agit le '.$done["dat_comt"].'</p>';
			  echo '<p>'.$done["commentaire"].'</p>';
			}
			$req1-> closeCursor()
				?>
                
                <form method="post" action="interm.php">
                     <div class="form-group" >
                     <label for="com">Commentaires</label>
                     <textarea cols="10" rows="3" id="com" name="com" class="form-control" required="required"> </textarea>
